...
class VLMWaypointPlannerSpeedCurvature(nn.Module):
    ...
    def __init__(self, name, api_model_name, api_base_url, api_key, **kwargs):
        ...
        if "llama" in self.model_path:
            ...
            self.model = Qwen2VLForConditionalGeneration.from_pretrained(
                "Qwen/Qwen2-VL-7B-Instruct", torch_dtype=torch.bfloat16, device_map="auto", cache_dir=cache_dir)
            ...
            self.image_token_se = DEFAULT_IM_START_TOKEN + \
        ...
        if "llama" in self.model_path:
            ...
            if "llama" in self.model_path:
                output_text = re.findall(
                    r'<\|start_header_id\|>assistant<\|end_header_id\|>(.*?)<\|eot_id\|>', output_text, re.DOTALL)[0].strip()
            return output_text

        elif "qwen" in self.model_path:
            message = self.getMessage(
                text, image=images, model_path=self.model_path)
    ...
    def _postprocess_result(self, result, init_x=0.0, init_y=0.0, init_yaw=0.0, dt=0.5):
        ...
        try:
            ...
            if json_match:
                ...
                json_str = json_match.group(1) if json_match.group(
        ...
        if not isinstance(speed_curv_list, list):
            raise ValueError(
                "The 'predicted_speeds_curvatures' should be a list of pairs.")
        ...
        for i, pair in enumerate(speed_curv_list):
            if not isinstance(pair, list) or len(pair) != 2:
                raise ValueError(
                    f"Expected each item to be [speed, curvature], got {pair} at index {i}.")
    ...
    def forward(self, perception_memory_bank, model_config) -> torch.Tensor:
        ...
        for i in range(N):
            # # if currently vehicle is in the air, then waypoint with static position
            # if perception_memory_bank[-1]["detmap_pose"][i][2] > 0.5:
            #     pred_waypoints = perception_memory_bank[-1]["detmap_pose"][i][:2].unsqueeze(0).repeat(FUT_LEN, 1)
            #     print(f"Vehicle is in the air, return static waypoints: {pred_waypoints}, height: {perception_memory_bank[-1]['detmap_pose'][i][2]}")
            ...
            front_image = \
                self._get_ego_front_image(
                    perception_memory_bank, i)  # (H, W, 3)
            # (2) prepare promt
    ...
    def forward_heter(self, perception_memory_bank, model_config, idx) -> torch.Tensor:
        ...
        pred_waypoints, _, _ = self._gen_individual_waypoints(front_image, ego_history_prompt, model_config, perception_memory_bank, idx)
