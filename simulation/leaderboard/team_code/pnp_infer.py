...
class PnP_infer():
	def __init__(self, config=None, ego_vehicles_num=1, perception_model=None, planning_model=None) -> None:
		...
		self.pre_raw_data_bank = queue.Queue()
	...
	def get_action_from_list_inter(self, car_data_raw, rsu_data_raw, step, timestamp):
		...
		self.pre_raw_data_bank.put(raw_data_dict)

		latency_step = self.config.latency_step

		if step >= latency_step:
			if self.pre_raw_data_bank.qsize() > latency_step+1:
				self.pre_raw_data_bank.get()
			elif self.pre_raw_data_bank.qsize() == latency_step+1:
				raw_data_used = self.pre_raw_data_bank.get()
