...
from team_code.vlm_infer_action import VLM_Infer
...
#### agents
class VLM_Agent(autonomous_agent.AutonomousAgent):
    """
    ...
    def setup(self, path_to_conf_file, ego_vehicles_num):
        ...
        self.infer = VLM_Infer(config=self.config,
    ...
    def tick(self, input_data: dict, vehicle_num: int):
        ...
        if "rgb_bev_{}".format(vehicle_num) in input_data:
            rgb_bev = cv.cvtColor(
                input_data["rgb_bev_{}".format(vehicle_num)][1][:, :, :3], cv.COLOR_BGR2RGB
            )
    ...
    def run_step(self, input_data: dict, timestamp):
        ...
        if self.config['planning']['core_method'] == 'MotionNet':
            control_all = self.infer.get_action_from_list_inter(car_data_raw=ego_data,
                                                            ...
                                                            step=self.step,
                                                            timestamp=timestamp)
        elif self.config['planning']['core_method'] == 'coopenemma':
            control_all = self.infer.get_action_from_list_inter(car_data_raw=ego_data,
                                                            rsu_data_raw=rsu_data,
                                                            step=self.step,
                                                            timestamp=timestamp)
